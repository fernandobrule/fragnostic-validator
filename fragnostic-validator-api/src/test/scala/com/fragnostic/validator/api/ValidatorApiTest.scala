...
class ValidatorApiTest extends AnyFunSpec with Matchers with BeforeAndAfterEach {
  ...
  describe("*** Validator Api Test ***") {
    ...
    it("Can Validate Yep") {
      val yep = "I am Yep"
      val validation: Validated[String] = yepValidator.validate(locale, "Yep", yep, params, List(yepValidatorEmptyMessage, yepValidatorFailMessage))
    ...
    it("Can Validate Wrong Number Of Arguments, Less Than") {
      val yep = "I am peY"
      val validation: Validated[String] = yepValidator.validate(locale, "Yep", yep, params, List(yepValidatorEmptyMessage))
      validation.isFailure should be(true)
    ...
    it("Can Validate Wrong Number Of Arguments, Great Than") {
      val yep = "I am peY"
      val validation: Validated[String] = yepValidator.validate(locale, "Yep", yep, params, List(yepValidatorEmptyMessage, yepValidatorEmptyMessage, yepValidatorEmptyMessage))
      validation.isFailure should be(true)
    ...
    it("Can Validate Yum") {
      val yum = "I am Yum"
      val params: Map[String, String] = Map[String, String]()
      val validation: Validated[String] = yumValidator.validate(locale, "Yum", yum, params, List(yumValidatorEmptyMessage, yumValidatorFailMessage))
      validation.isSuccess should be(true)
    ...
    it("Can Validate Gi") {
      val gi = "I am Gi"
      val validation: Validated[String] = giValidator.validate(locale, "Gi", gi, params, List(giValidatorEmptyMessage, giValidatorFailMessage))
      validation.isSuccess should be(true)
